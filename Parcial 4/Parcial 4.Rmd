...
Verticalmente se tiene un año representado. Por año, nuevamente Marzo siempre destaca por ser el mes con mayor numero de sacrificios. 
...
De un total de 256 modelos se seleccionan:
...
siguiente paso con los 256 modelos. Se importan 256 modelos previamente ajustados. Se seleccionan los menores AIC y BIC. 
...
f_fit <- forecast(prueba)
 autoplot(dftimeserie, series="Datos") + 
   autolayer(prueba$fitted, series="SARIMA(0,1,1)(0,1,0)[12] ") +
...
```{r}
p2 = auto.arima(df2$t_cabezas,stepwise = F,approximation = F,seasonal = 12)
```
...
autoplot(ts(df2$t_cabezas), series="Datos") + 
   autolayer(p2$fitted, series="Modelo auto.arima ") +
   autolayer(f_fit, series="Prediction") +
   xlab("Year") + ylab("Total sacrificios (1/100.000)") + ggtitle("Total de sacrifios 2008-2022") + theme_bw()+theme(legend.title = element_blank(),legend.position = "bottom")

```

Note que la prediccion del modelo de auto.arima es mas pobre. Esto dado que; hace falta realizar un ajuste de estacionalidad. 
...
Observe de nuevo la serie original.
